(ns fao_stat.main
  "Kotodama WASM entrypoint for the fao_stat clean-room actor (L4) — Clojure port.
  ...
  facts (`fao_stat.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fao_stat")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"fao_stat.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fao_stat-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fao_stat-compat/ready)
